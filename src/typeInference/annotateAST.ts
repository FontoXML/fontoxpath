...
export function countQueryBodyAnnotations(
	...
	if (annotationFunctions.has(ast[0]) && ast[0] !== 'queryBody') {
...
function annotate(ast: IAST, context: AnnotationContext): SequenceType | undefined {
	...
	const annotationFunction = annotationFunctions.get(astNodeName);
	if (annotationFunction) {
		return annotationFunction(ast, context);
	}
 ...
 */
const annotationFunctions = new Map<
	string,
...
>([
	...
	[
		'integerConstantExpr',
		(ast: IAST, _context: AnnotationContext): SequenceType => {
