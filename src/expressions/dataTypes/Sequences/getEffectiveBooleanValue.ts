...
export default function getEffectiveBooleanValue(value: Value): boolean {
	...
	if (isSubtypeOf(value.type, { kind: BaseType.XSBOOLEAN, seqType: SequenceType.EXACTLY_ONE })) {
	...
	if (
		...
		isSubtypeOf(value.type, { kind: BaseType.XSANYURI, seqType: SequenceType.EXACTLY_ONE }) ||
