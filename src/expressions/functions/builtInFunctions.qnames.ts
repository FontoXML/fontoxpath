...
const fnQName:FunctionDefinitionType = function(_dynamicContext, _executionParameters, _staticContext, paramURI, paramQName) {
	return zipSingleton([paramURI, paramQName], ([uriValue, lexicalQNameValue]) => {
		...
		return SequenceFactory.singleton(createAtomicValue(new QName(prefix, uri, localName), 'xs:QName'));
	});
}

const fnPrefixFromQName:FunctionDefinitionType = function(_dynamicContext, _executionParameters, _staticContext, arg) {
...
const fnNamespaceURIFromQName:FunctionDefinitionType = function(_dynamicContext, _executionParameters, _staticContext, arg) {
...
const fnLocalNameFromQName:FunctionDefinitionType = function(_dynamicContext, _executionParameters, _staticContext, arg) {
	return arg.map(qname => {
		const qnameValue = qname.value;
