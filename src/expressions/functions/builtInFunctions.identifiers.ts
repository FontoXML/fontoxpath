...
const fnId: FunctionDefinitionType = function(_dynamicContext, executionParameters, _staticContext, idrefSequence, targetNodeSequence) {
	...
	const isMatchingIdById: {[s:string]: boolean} = idrefSequence.getAllValues().reduce(function (byId, idrefValue) {
...
const fnIdref: FunctionDefinitionType = function(_dynamicContext, executionParameters, _staticContext, idSequence, targetNodeSequence) {
	const targetNodeValue = targetNodeSequence.first();
	if (!isSubtypeOf(targetNodeValue.type, 'node()')) {
		return SequenceFactory.empty();
	}
