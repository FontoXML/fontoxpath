...
>(
	...
	domFacadeLocal?: IDomFacade | null,
	...
	if (!cachedCompiledXPath) {
		...
		if (compiledXPathResult.isAstAccepted === true) {
			...
			return executeJavaScriptCompiledXPath(evalFunction, contextItem, domFacadeLocal);
		} else {
			throw new Error(compiledXPathResult.reason);
		}
	} else {
		return executeJavaScriptCompiledXPath(cachedCompiledXPath, contextItem, domFacadeLocal);
	}
};
