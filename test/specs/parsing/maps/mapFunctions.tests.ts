...
describe('functions over maps', () => {
	describe('map:get', () => {
		it('can get a value from a map using a string key', () =>
			chai.assert.equal(
				evaluateXPathToNumber('map:get(map {"a": 1, "b":2}, "a")', documentNode),
				1,
			));
		...
		it('can get a value from a map multiple times', () =>
			chai.assert.equal(
				evaluateXPathToNumber(
					'let $map := map {"a": (1,2,3,4)} return count($map("a")) + count($map("a") => reverse())',
					documentNode,
				),
				8,
			));
		...
		it('can get a value from a map using a boolean key', () =>
			chai.assert.equal(
				evaluateXPathToNumber('map:get(map {true(): 1, false():2}, true())', documentNode),
				1,
			));

		it('can get a value from a map using a numeric key', () =>
			chai.assert.equal(
				evaluateXPathToNumber('map:get(map {1: 1, 2:2}, 1)', documentNode),
				1,
			));

		it('returns the empty sequence if nothing matches: numeric vs other', () =>
			chai.assert.equal(
				evaluateXPathToNumber('count(map:get(map {1: 1, 2:2}, false()))', documentNode),
				0,
			));

		it('returns the empty sequence if nothing matches: other vs numeric', () =>
			chai.assert.equal(
				evaluateXPathToNumber('count(map:get(map {true(): 1, 2:2}, 1))', documentNode),
				0,
			));
		...
		it('returns the correct value when passes NaN', () =>
			chai.assert.equal(
				evaluateXPathToNumber(
					'(map {number("NaN"): 1, "b":2})(number("NaN"))',
					documentNode,
				),
				1,
			));
...
map:merge((
  map {"a": 1},
  map {"a": 2}
), map {"duplicates": "combine"}) => deep-equal(map{"a": (1,2)})`,
					documentNode,
				),
			));
  ...
  map {"a": 2}
	...
	describe('map:put', () => {
		it('adds an item to a map', () =>
			chai.assert.deepEqual(
				evaluateXPathToMap('map{"a": 1} => map:put("b", 2)', documentNode),
				{ a: 1, b: 2 },
			));
		...
		it('does not mutate the map', () =>
			chai.assert.deepEqual(
				evaluateXPathToMap(
					'let $mapA := map{"a": 1}, $mapB := map:put($mapA, "a", 2) return $mapA',
					documentNode,
				),
				{ a: 1 },
			));
	...
	describe('map:keys', () => {
		...
		it('returns the keys for a map with values', () =>
			chai.assert.deepEqual(
				evaluateXPathToStrings('map:keys(map{"a":1, "b":2})', documentNode),
				['a', 'b'],
			));
		it('returns the keys of merged maps', () =>
			chai.assert.isTrue(
				evaluateXPathToBoolean(
					'let $result := map:keys(map:merge((map:entry("a", "1"), map:entry("b", 2)))) return $result = "a"',
					documentNode,
				),
			));
	});
	...
	describe('map:remove', () => {
		...
		it('removes multiple items if multiple keys are passed', () =>
			chai.assert.deepEqual(
				evaluateXPathToMap(
					'map:remove(map{"a":1, "b": 2, "c": 3}, ("a", "b"))',
					documentNode,
				),
				{ c: 3 },
			));
