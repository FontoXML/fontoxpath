...
describe('map constructor', () => {
	...
	it('is parsed using longest substring (map{x:a:b} is map{{(x:a):b})', () => {
		...
		element
			.appendChild(documentNode.createElementNS('xxx', 'x:a'))
			.appendChild(documentNode.createTextNode('a'));
		element
			.appendChild(documentNode.createElementNS('aaa', 'a:b'))
			.appendChild(documentNode.createTextNode('a:b'));
