...
import { evaluateXPath, evaluateXPathToNodes, evaluateXPathToString, registerXQueryModule } from 'fontoxpath';
...
describe.skip('xquerydoc', () => {
	...
	it('run xQueryDoc parser over simple XQuery', () => {
		...
		const expectedResult =
			`<doc:xqdoc xmlns:doc="http://www.xqdoc.org/1.0">
				<doc:control><!--Generated by xquerydoc: http://github.com/xquery/xquerydoc-->
					<doc:date>2019-09-06T07:36:38.178Z</doc:date>
		...
		chai.assert.equal((result[0] as slimdom.Element).outerHTML, expectedResult);
	});
	...
	it('run XQuery parser over example from the existDB docs', () => {
		...
		chai.assert.equal((result[0] as slimdom.Element).outerHTML, `
