...
describe('IfExpression', () => {
	...
	it('Allows async conditions', async () => {
		chai.assert.isTrue(
			await evaluateXPathToAsyncSingleton(
				'(if (fontoxpath:sleep(true(), 1)) then "then expression" else "else expression") eq "then expression"',
				documentNode
			)
		);
	});
	...
	it('Allows async thenExpression', async () => {
		chai.assert.isTrue(
			await evaluateXPathToAsyncSingleton(
				'(if (true()) then fontoxpath:sleep("then expression", 1) else "else expression") eq "then expression"',
				documentNode
			)
		);
	});
	...
	it('Allows async elseExpression', async () => {
		chai.assert.isTrue(
			await evaluateXPathToAsyncSingleton(
				'(if (false()) then "then expression" else fontoxpath:sleep("else expression", 1)) eq "else expression"',
				documentNode
			)
		);
	});
