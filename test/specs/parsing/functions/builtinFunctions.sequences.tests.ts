	...
	describe('General functions and operators on sequences', () => {
		...
		describe('fn:subsequence', () => {
			...
			it('allows async parameters: sequence', async () => {
				chai.assert.equal(
					await evaluateXPathToAsyncSingleton(
						'subsequence(("a", "b", "c") => fontoxpath:sleep(1), 2, 1)'
					),
					'b'
				);
			});
			it('allows async parameters: async items in sequence', async () => {
				chai.assert.equal(
					await evaluateXPathToAsyncSingleton(
						'subsequence(("a" => fontoxpath:sleep(1), "b" => fontoxpath:sleep(10), "c"), 2, 1)'
					),
					'b'
				);
			});
			it('allows async parameters: from', async () => {
				chai.assert.equal(
					await evaluateXPathToAsyncSingleton(
						'subsequence(("a", "b", "c"), 2 => fontoxpath:sleep(1), 1)'
					),
					'b'
				);
			});
			it('allows async parameters: length', async () => {
				chai.assert.equal(
					await evaluateXPathToAsyncSingleton(
						'subsequence(("a", "b", "c"), 2, 1 => fontoxpath:sleep(1))'
					),
					'b'
				);
			});
  ...
  <matrix xmlns="http://example.com/ns1" frame="all"><tgroup xmlns="http://example.com/ns2" cols="1"><colspec colname="column-0" colnum="1" colwidth="1*" colsep="1" rowsep="1"/><tbody><row><entry colname="column-0" colsep="1" rowsep="1"/></row></tbody></tgroup></matrix>
		...
		describe('async params', () => {
			it('returns true if both sequences are empty', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(() => fontoxpath:sleep(1), () => fontoxpath:sleep(1))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal string', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(("abc") => fontoxpath:sleep(1), ("abc") => fontoxpath:sleep(1))',
						documentNode
					)
				);
			});
			...
			it('returns false if both sequences are a function', async () => {
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(true#0 => fontoxpath:sleep(1), true#0 => fontoxpath:sleep(1))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal number (1 vs 1)', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((1) => fontoxpath:sleep(1), (1) => fontoxpath:sleep(1))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal number (1 vs float 1)', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((1) => fontoxpath:sleep(1), (xs:float("1") => fontoxpath:sleep(1)))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal number (float NaN vs float NaN)', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((xs:float("NaN") => fontoxpath:sleep(1)), (xs:float("NaN") => fontoxpath:sleep(1)))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal number (double 1 vs 1)', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((xs:double("1") => fontoxpath:sleep(1)), (1) => fontoxpath:sleep(1))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal number (xs:int and xs:int)', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((xs:int("-2147483648")) => fontoxpath:sleep(1), (xs:int("-2147483648") => fontoxpath:sleep(1)))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal number (double 1 vs float 1)', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((xs:double("1") => fontoxpath:sleep(1)), (xs:float("1") => fontoxpath:sleep(1)))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal number (double 1 vs double 1)', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((xs:double("1") => fontoxpath:sleep(1)), (xs:double("1") => fontoxpath:sleep(1)))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal number (double NaN vs double NaN)', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((xs:double("NaN") => fontoxpath:sleep(1)), (xs:double("NaN") => fontoxpath:sleep(1)))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal boolean', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((xs:boolean("true") => fontoxpath:sleep(1)), xs:boolean("true") => fontoxpath:sleep(1))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal set of values', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(("abc" => fontoxpath:sleep(1), 1, xs:boolean("false")), ("abc", 1 => fontoxpath:sleep(1), xs:boolean("false")))',
						documentNode
					)
				);
			});
			...
			it('returns false if both sequences have a different length', async () => {
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(("xyz", "abc" => fontoxpath:sleep(1)), ("abc"))',
						documentNode
					)
				);
			});
			...
			it('returns false if both sequences contain different strings', async () => {
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(("xyz" => fontoxpath:sleep(1)), ("abc") => fontoxpath:sleep(1))',
						documentNode
					)
				);
			});
			...
			it('returns false if both sequences contain different numbers', async () => {
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((22 => fontoxpath:sleep(1)), (42 => fontoxpath:sleep(1)))',
						documentNode
					)
				);
			});
			...
			it('returns false if both sequences contain different booleans', async () => {
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((xs:boolean("true") => fontoxpath:sleep(1)), (xs:boolean("false") => fontoxpath:sleep(1)))',
						documentNode
					)
				);
			});
			...
			it('returns false if both sequences contain multiple different values', async () => {
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(("abc", 12 => fontoxpath:sleep(1), xs:boolean("false"), xs:boolean("true")), ("abc", 12, xs:boolean("false"), xs:boolean("false")))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal map', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((map { "a": 123, "b": 456 => fontoxpath:sleep(1) }), (map { "a": 123, "b": 456 } => fontoxpath:sleep(1)))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal map with items on different positions', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((map { "b" => fontoxpath:sleep(1): 456, "a": 123 }), (map { "a": 123, "b": 456 } => fontoxpath:sleep(1)))',
						documentNode
					)
				);
			});
			...
			it('returns true if both sequences contain an equal array', async () => {
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(([1, 2, 5, 8] => fontoxpath:sleep(1)), ([1, 2, 5, 8]))',
						documentNode
					)
				);
			});
			...
			it('returns false if both sequences contain maps with a different length', async () => {
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((map { "a": 123, "b": 456 }), (map { "a": 123, "b": 456, "c": 789 => fontoxpath:sleep(10) }))',
						documentNode
					)
				);
			});
			...
			it('returns false if both sequences contain maps with different keys', async () => {
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((map { "a": 123, "b" => fontoxpath:sleep(1): 456 }), (map { "y": 123, "z": 456 }))',
						documentNode
					)
				);
			});
			...
			it('returns false if both sequences contain arrays with a different length', async () => {
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(([1, 2, 5, 8, 10] => fontoxpath:sleep(1)), ([1, 2, 5, 8] => fontoxpath:sleep(1)))',
						documentNode
					)
				);
			});
			...
			it('returns true for the same element', async () => {
				...
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(./someElement, ./someElement => fontoxpath:sleep(1))',
						documentNode
					)
				);
			});
			it('returns true two equal elements', async () => {
				...
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'deep-equal((./someElement => fontoxpath:sleep(1))/someEqualElement[1], ./someElement/someEqualElement[2])',
						documentNode
					)
				);
			});
			it('returns true two unequal elements (unequal on node name)', async () => {
				...
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(./someElement/someEqualElement => fontoxpath:sleep(1), ./someElement/someUnequalElement)',
						documentNode
					)
				);
			});
			it('returns true two unequal elements (unequal on contents)', async () => {
				...
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'deep-equal(./someElement/someEqualElement[1], ./someElement/someEqualElement[2] => fontoxpath:sleep(1))',
						documentNode
					)
				);
			});
			...
			it('returns true for two sequences containing two equal attribute nodes', async () => {
				...
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'let $attributeNode := ./someElement/someElement/@someAttribute return deep-equal($attributeNode[1] => fontoxpath:sleep(1), $attributeNode[2])',
						documentNode
					)
				);
			});
			it('returns false for two sequences containing two unequal attribute nodes', async () => {
				...
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'let $attributeNode := ./someElement/someElement/@someAttribute return deep-equal($attributeNode[1], $attributeNode[2] => fontoxpath:sleep(1))',
						documentNode
					)
				);
			});
			...
			it("returns true for two sequences containing two equal PI's", async () => {
				...
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'let $piNode := ./someElement/processing-instruction() return deep-equal($piNode[1], $piNode[2] => fontoxpath:sleep(1))',
						documentNode
					)
				);
			});
			it("returns false for two sequences containing two unequal PI's", async () => {
				...
				chai.assert.isFalse(
					await evaluateXPathToAsyncSingleton(
						'let $piNode := ./someElement/processing-instruction() return deep-equal($piNode[1] => fontoxpath:sleep(1), $piNode[2])',
						documentNode
					)
				);
			});
			...
			it('returns true for two sequences containing two equal text nodes', async () => {
				...
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'let $textNode := ./someElement/text() => fontoxpath:sleep(1) return deep-equal($textNode[1], $textNode[2])',
						documentNode
					)
				);
			});
			...
			it('returns true for comments', async () => {
				...
				chai.assert.isTrue(
					await evaluateXPathToAsyncSingleton(
						'let $commentNode := ./someElement/comment() => fontoxpath:sleep(1) return deep-equal($commentNode[1], $commentNode[2])',
						documentNode
					)
				);
			});
