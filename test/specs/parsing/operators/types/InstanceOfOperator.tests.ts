...
describe('instance of operator', () => {
	...
	it('accepts async params', async () => {
		chai.assert.isTrue(
			await evaluateXPathToAsyncSingleton('fontoxpath:sleep(1) instance of xs:integer')
		);
	});
	it('accepts async params for * case, single frame', async () => {
		chai.assert.isTrue(
			await evaluateXPathToAsyncSingleton('fontoxpath:sleep((1,2,3)) instance of xs:integer*')
		);
	});
	it('accepts async params for + case, multiple frames', async () => {
		chai.assert.isTrue(
			await evaluateXPathToAsyncSingleton(
				'(fontoxpath:sleep(1, 1), fontoxpath:sleep(2, 10)) instance of xs:integer+'
			)
		);
	});
	it('accepts async params for + case, false case', async () => {
		chai.assert.isFalse(
			await evaluateXPathToAsyncSingleton(
				'(fontoxpath:sleep(1, 1), fontoxpath:sleep("string", 10)) instance of xs:integer+'
			)
		);
	});
});
