...
describe('Filter', () => {
	...
	describe('Filter.getBucket()', () => {
		it('returns the bucket of its selector', () => {
			const filter = new Filter({ getBucket: () => 'bucket', specificity: new Specificity({}) } as Expression, equalExpression);
	...
	describe('Filter.specificity', () => {
		it('returns the specificity of the selector plus the other part', () => {
			const filter = new Filter({ specificity: new Specificity({ external: 1 }) } as Expression, { specificity: new Specificity({ external: 1 }) } as Expression);
